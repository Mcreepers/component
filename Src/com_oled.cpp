...
com_oled::com_oled() : Show(*this) {
    ...
    OLED_Buf_Ptr = (uint8_t **)OLED_Buf;  // OLED_Buf_Ptr point to OLED_Buf
}
...
uint8_t com_oled::Set_Screen_Type(Screen_type type) {
    this->type = type;
    return COM_OK;
}
...
uint8_t com_oled::Begin() {
    ...
    ret += WR_Byte(0xAE, 1);  // 设置显示开启/关闭，0xAE关闭，0xAF开启

    ret += WR_Byte(0xD5, 1);  // 设置显示时钟分频比/振荡器频率
    ret += WR_Byte(0x80, 1);  // 0x00~0xFF

    ret += WR_Byte(0xA8, 1);  // 设置多路复用率
    ret += WR_Byte(0x3F, 1);  // 0x0E~0x3F

    ret += WR_Byte(0xD3, 1);  // 设置显示偏移
    ret += WR_Byte(0x00, 1);  // 0x00~0x7F

    ret += WR_Byte(0x40, 1);  // 设置显示开始行，0x40~0x7F

    ret += WR_Byte(0xA1, 1);  // 设置左右方向，0xA1正常，0xA0左右反置

    ret += WR_Byte(0xC8, 1);  // 设置上下方向，0xC8正常，0xC0上下反置

    ret += WR_Byte(0xDA, 1);  // 设置COM引脚硬件配置
    ret += WR_Byte(0x12, 1);

    ret += WR_Byte(0x81, 1);  // 设置对比度
    ret += WR_Byte(0xCF, 1);  // 0x00~0xFF

    ret += WR_Byte(0xD9, 1);  // 设置预充电周期
    ret += WR_Byte(0xF1, 1);

    ret += WR_Byte(0xDB, 1);  // 设置VCOMH取消选择级别
    ret += WR_Byte(0x30, 1);

    ret += WR_Byte(0xA4, 1);  // 设置整个显示打开/关闭

    ret += WR_Byte(0xA6, 1);  // 设置正常/反色显示，0xA6正常，0xA7反色

    ret += WR_Byte(0x8D, 1);  // 设置充电泵
    ret += WR_Byte(0x14, 1);

    ret += WR_Byte(0xAF, 1);  // 开启显示
...
uint8_t com_oled::Begin_Fast() {
    ...
    ret += WR_Byte(0xAE, 1);  // Set display on/off
    ret += WR_Byte(0xA6, 1);  // Set normal/reverse display

    Clear();   // Clear OLED_Buf
 ...
 * @param Width: x end
 * @param Height: y end
 * @note To display, use Update() after this function
 */
void com_oled::Clear(uint8_t x, uint8_t y, uint8_t Width, uint8_t Height) {
    if (x > 127 || y > 63) {
        return;
    }
    if (x + Width > 128) {
...
uint8_t com_oled::Update() {
    ...
    for (uint8_t i = 0; i < 8; i++) {
        ret += Set_Cursor(i, 0);
        ret += WR_Data((uint8_t *)OLED_Buf[i], 128);
    }
    if (ret != COM_OK) {
        return COM_ERROR;
    }
 ...
 */
uint8_t com_oled::Update(uint8_t x, uint8_t y, uint8_t Width, uint8_t Height) {
    if (OLED_Fresh == 0) {
        return COM_OK;
    }
    OLED_Fresh = 0;
    ...
    for (uint8_t i = y / 8; i < (y + Height) / 8 + 1; i++) {
        ret += Set_Cursor(i, x);
        ret += WR_Data(&OLED_Buf_Ptr[i][x], Width);
    }
    if (ret != COM_OK) {
        return COM_ERROR;
    }
...
/// @param y
/// @return
uint8_t com_oled::SetPos(uint8_t x, uint8_t y) {
    uint8_t ret = COM_OK;
    return ret;
}
 ...
 * @param
 */
uint8_t com_oled::Fill(uint8_t bmp_data) {
    uint8_t ret = COM_OK;
    return ret;
}
...
uint8_t com_oled::show::Char(uint8_t x, uint8_t y, uint8_t chr, uint8_t Size) {
    switch (Size) {
        case Font_6X8:
            oled.ShowImage(x, y, 6, 8, (uint8_t *)&asc2_0608[chr - ' ']);
            break;
        case Font_8X16:
            oled.ShowImage(x, y, 8, 16, (uint8_t *)&asc2_0816[chr - ' ']);
            break;
        case Font_12X12:
            oled.ShowImage(x, y, 12, 12, (uint8_t *)&asc2_1212[chr - ' ']);
            break;
        case Font_16X16:
            oled.ShowImage(x, y, 16, 16, (uint8_t *)&asc2_1616[chr - ' ']);
            break;
        case Font_24X24:
            oled.ShowImage(x, y, 24, 24, (uint8_t *)&asc2_2424[chr - ' ']);
            break;
        case Font_CHINESE:
            oled.ShowImage(x, y, 16, 16, (uint8_t *)&Chinese1616[chr]);
        default:
            return COM_ERROR;
            break;
    }
...
uint8_t com_oled::show::Chinese(uint8_t x, uint8_t y, char *Chinese) {
    ...
    for (uint8_t i = 0; i < CHN_CHAR_WIDTH; i++) {
        ...
        if (pChinese >= CHN_CHAR_WIDTH) {
            ...
            // pIndex < 1300;pIndex++) {}
            oled.ShowImage(x + ((i + 1) / CHN_CHAR_WIDTH - 1) * 16, y, 16, 16,
                           (uint8_t *)&Chinese1616[pIndex]);
        }
    }
 ...
 */
uint8_t com_oled::Set_Cursor(uint8_t page, uint8_t x) {
    uint8_t ret = COM_OK;
    x += 0x02;
    ret += WR_Byte(0xb0 | page, 1);
    ret += WR_Byte(0x10 | (x & 0xf0) >> 4, 1);
    ret += WR_Byte(0x00 | (x & 0x0f), 1);
    if (ret != COM_OK) {
        return COM_ERROR;
    }
    return ret;
...
// reverse OLED_Buf.To display, use Update() after this function
void com_oled::Reverse() {
    for (uint8_t i = 0; i < 8; i++) {
 ...
 * @note To display, use Update() after this function
 */
void com_oled::Reverse(uint8_t x, uint8_t y, uint8_t Width, uint8_t Height) {
    if (x > 127 || y > 63) {
        return;
    }
    if (x + Width > 128) {
...
uint8_t com_oled::ShowImage(uint8_t x, uint8_t y, uint8_t Width, uint8_t Height,
                            const uint8_t *Image) {
    if (x > 127 || y > 63) {
        return COM_ERROR;
    }
 ...
 */
uint8_t com_oled::WR_Byte(uint8_t data, uint8_t cmd) {
    uint8_t ret = COM_OK;
 ...
 */
uint8_t com_oled::WR_Data(uint8_t *data, uint8_t len) {
    uint8_t ret = COM_OK;
    if (hardware == Hardware_IIC) {
#if defined(HAL_I2C_MODULE_ENABLED)
...
// wait for a ack
void com_oled::I2C_WaitAck() {
    WR_SCL(1);
    WR_SCL(0);
}

#if defined(HAL_I2C_MODULE_ENABLED)
uint8_t com_oled::I2C_Transmit(uint8_t *data, uint16_t len) {
...
#endif
// set SDA
void com_oled::WR_SDA(uint8_t data) {
...
// set SCL
void com_oled::WR_SCL(uint8_t data) {
