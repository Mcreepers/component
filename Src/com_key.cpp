 ...
 */
bool com_key::Get_Key_Press() { return this->Key_Press; }
...
void com_key::Key_IC_ISR() {
    ...
    if (this->Mode == 2) {
        if (edge_state == false) {
            edge_state = true;
            Set_Key_Press(true);
            __HAL_TIM_SET_CAPTUREPOLARITY(this->htim, this->Channel,
...
void com_key::Key_Stroke() {
    ...
    if (HardWare == 0) {
        ...
        if (this->Key_Press == true && this->Mode == 0 &&
            HAL_GPIO_ReadPin(this->GPIOx, this->GPIO_Pin) == 0) {
            count++;
        } else if (this->Key_Press == true && this->Mode == 1 &&
                   HAL_GPIO_ReadPin(this->GPIOx, this->GPIO_Pin) == 1) {
            count++;
        } else {
            Set_Key_Press(false);
            count = 0;
