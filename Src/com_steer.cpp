...
{
	this->angle = angle;
	this->duty = roundf((angle * 10 + 500) / 1800 * duration + 500) + offset_duty;
	return this->duty;
}

float com_steer::Get_Steer_Angle()
{
	return this->angle;
}

uint16_t com_steer::Get_Steer_Duty()
{
	return this->duty;
}
...
void com_steer::Set_Steer_Angle(float angle)
{
	...
	{
		this->duty = roundf(angle / 1800 * duration + 185 * duration / 200) + offset_duty;
	}
	else
	{
		this->duty = roundf(angle / 1800 * duration + 15 * duration / 200) + offset_duty;
	}
}
