...
com_mpu6050::com_mpu6050(I2C_HandleTypeDef *hi2c) { this->hi2c = hi2c; }
...
uint8_t com_mpu6050::Get_MPU6050_Data(MPU6050_Data_t *pData) {
    if (pData == NULL || Data.Address == NULL) {
        return COM_ERROR;
    }
...
void Quaternion_IMU_c::Calc(float gx, float gy, float gz, float ax, float ay,
                            float az) {
    float norm;
    float vx, vy, vz;
    float ex, ey, ez;
...
void Kalman_IMU_c::Kalman_Data_t::Kalman_Update(float newAngle, float newRate,
                                                float dt) {
    // 预测
    rate = newRate - bias;
    angle += dt * rate;
...
void Kalman_IMU_c::Calc(float gx, float gy, float gz, float ax, float ay,
                        float az) {
    float accel_x = ax;
    float accel_y = ay;
    float accel_z = az;
    ...
    //
    float accel_angle_x =
        atan2(accel_y, sqrt(accel_x * accel_x + accel_z * accel_z)) * 180 / PI;
    float accel_angle_y =
        atan2(accel_x, sqrt(accel_y * accel_y + accel_z * accel_z)) * 180 / PI;
    float accel_angle_z = atan2(accel_y, accel_x) * 180 / PI;

    //
    float Angle_X_Final =
        0.98 * (accel_angle_x + gx * dt) + 0.02 * accel_angle_x;
    float Angle_Y_Final =
        0.98 * (accel_angle_y + gy * dt) + 0.02 * accel_angle_y;
    float Angle_Z_Final =
        0.98 * (accel_angle_z + gz * dt) + 0.02 * accel_angle_z;
