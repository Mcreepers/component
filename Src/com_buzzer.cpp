...
com_buzzer::com_buzzer(GPIO_TypeDef *GPIOx, uint16_t GPIO_PinState,
                       uint16_t aTime)
...
void com_buzzer::Delay(uint32_t ms) { com_delay_ms_os(ms); }
...
uint8_t com_buzzer::Play_Music(song_type music) {
  ...
  case Always_with_me:
    for (int i = 0;
         i < sizeof(Always_with_me_note) / sizeof(Always_with_me_note[0]);
         i++) {
      Set_Music_PWM(trans_note_to_freq(Always_with_me_note[i].mName),
                    Always_with_me_note[i].mTime);
    }
    break;
...
uint8_t com_buzzer::Play_Music_Once(song_type music) {
  ...
  case Always_with_me:
    ...
    Set_Music_PWM(trans_note_to_freq(Always_with_me_note[count].mName),
                  Always_with_me_note[count].mTime);
    break;
