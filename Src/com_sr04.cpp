 ...
 * @note Must use SR04TimOverflowIsr in HAL_TIM_PeriodElapsedCallback,use
 * SR04_Tim_IC_Isr in TIMx_IC_IRQHandler.Otherwise, it will not work.
 */
com_sr04::com_sr04(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                   TIM_HandleTypeDef *htim, uint8_t Tim_Channel,
                   uint32_t *Tim_overflow_Cnt)
    : GPIOx(GPIOx),
      GPIO_Pin(GPIO_Pin),
      htim(htim),
      Tim_Channel(Tim_Channel),
      Tim_overflow_Cnt(Tim_overflow_Cnt) {}
...
void com_sr04::SR04TimOverflowIsr() { (*Tim_overflow_Cnt)++; }
...
void com_sr04::SR04_Tim_IC_Isr() {
    if (edge_state == 0) {
        ...
        *Tim_overflow_Cnt = 0;
        ...
        Distance = (Duration * 0.034 / 2) * Iterative +
                   Distance_temp * (1 - Iterative);
