...
enum Uart_StatusTypeDef { RX_RXNE = 0, RX_IDLE, RX_RXNE_IDLE, RX_ERROR };
...
class com_uart {
   ...
   public:
    com_uart(UART_HandleTypeDef *huart_, Uart_StatusTypeDef Uart_State_)
        : huart(huart_), Uart_Set_State(Uart_State_) {
        Uart_Len = 0;
    };
    ...
    uint16_t Get_Uart_Len_Clear() {
        uint16_t len = Uart_Len;
        Uart_Len = 0;
        return len;
    };
