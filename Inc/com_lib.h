...
template <typename T>
void Find_Max_Min(T *data, uint32_t len, T &max,
                  T &min);  // find the max and min value
template <typename T, typename T_mem, T_mem(T::*mem)>
void Find_Max_Min(
    T *address, uint8_t len, T &max,
    T &min);  // find the max and min value by the member of the class
template <typename T>
void Insertion_Sort(T *data, uint32_t len);  // sort the data
// template <typename T, typename T_mem, T_mem(T:: *mem)>
// void Insertion_Sort(T *address, uint8_t len);//sort the data by the member of
// the class
}  // namespace com_lib
